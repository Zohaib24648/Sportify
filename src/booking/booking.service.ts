    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          'Cannot update booking in its current status',
        );
      }
    ...
    try {
      ...
      return this.prisma.$transaction(async (prisma) => {
        ...
        for (const payment of payments) {
          const statusUpdate =
            payment.payment_status === 'paid' ? 'refund_pending' : 'refunded';
    ...
    try {
      ...
      if (searchTerm) {
        where.OR = [
          ...
          {
            slot: {
              court: { name: { contains: searchTerm, mode: 'insensitive' } },
            },
          },
        ];
      }
