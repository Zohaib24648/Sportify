...
export class SlotService {
  ...
  async checkOverlappingSlot(dto: SlotDto, excludeSlotId?: string): Promise<boolean> {
    try {
    ...
    try {
      ...
      for (const date of dates) {
        ...
        for (let i = 1; i < availabilityIntervals.length; i++) {
          ...
          if (currentInterval.end.isAfter(nextInterval.start) || currentInterval.end.isSame(nextInterval.start)) {
            // Merge intervals
        ...
        if (!isSlotFullyCovered) {
          console.error(`Slot time is outside court availability hours on ${dayName}`);
          return false;
        }
      }
  ...
  async getAvailableSlotsForDay(dto: { court_id: string; date: string }) {
    ...
    const dayName = dayjs.tz(date, 'Asia/Karachi').format('dddd').toLowerCase() as DAY;
    ...
    if (!availabilities.length) {
      console.error(`No availability found for court ${court_id} on ${dayName}`);
      throw new NotFoundException(`No availability found for this court on the given date`);
    }
    ...
    closurePeriods.forEach((closure) => {
      ...
      for (let i = 0; i < mergedIntervals.length; i++) {
        ...
        if (closureEnd.isBefore(interval.start) || closureStart.isAfter(interval.end)) {
          continue; // No overlap
        }
